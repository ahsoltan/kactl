...
struct CT {
  vi sub, cp, d;
  vector<vi> g, dst;
  CT(vector<vi> G) : sub(sz(G)), cp(sz(G),-2), d(sz(G)), g(G) {
    dst.resize(__lg(sz(G)) + 1, vi(sz(G)));
    rec(0, 0);
  }
  ...
  void gen(int u, int p, int lev) {
    ...
    for (int v : g[u]) if (v!=p && cp[v]==-2) gen(v, u, lev);
  }
  int rec(int u, int dd) {
    dfs(u, -1);
    int p = -1, s = sub[u]; rep:
    for (int v : g[u]) if (v!=p && cp[v]==-2 && sub[v]>s/2) {
      p = u, u = v; goto rep;
    }
...
struct ContourAdd : CT {
  ...
  // Add x to all vertices whose distance from p is in [l, r)
  void add(int p, int l, int r, int x) {
    path(p, [&](int u, int v, int dd) {
      d[u].update(max(0, l - dd), x);
      if (r - dd < sub[u]) d[u].update(max(0, r - dd), -x);
      if (v != -1) {
        c[v].update(max(0, l - dd), x);
        if (r - dd < sub[u]) c[v].update(max(0, r - dd), -x);
      }
    });
  }
  ll get(ll p) {
